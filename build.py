...
"""
Script to build libntl for RACE
"""
...
def get_cli_arguments():
    ...
    parser = builder.get_arg_parser("ntl", "11.4.4", 1, __file__, [builder.TARGET_LINUX_x86_64, builder.TARGET_LINUX_arm64_v8a])
...
if __name__ == "__main__":
    ...
    builder.install_packages(args, [
        ("libgmp3-dev", "2:6.2.*", True),
    ])

    builder.fetch_source(
        args=args,
        source=f"https://libntl.org/ntl-{args.version}.tar.gz",
        extract="tar.gz",
    )

    source_dir = os.path.join(args.source_dir, f"ntl-{args.version}", "src")
